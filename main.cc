...
int main(int, char**)
{
	...
	std::fstream file;
	file.open("music/The Legend of Zelda Ocarina of Time - Kokiri Forest.mid", std::ios::in | std::ios::binary);
	...
	std::cout << "MIDI файл, формат: " <<  mthd.format <<
			", дорожек: " << mthd.ntrks <<
			...
			case 0xff:{
				...
				switch (meta_type) {
					...
					case 0x21:{
						unsigned prefix;
						prefix = track[trk][ptr++];
						std::cout << "midi_port: " << prefix << std::endl;
					}break;
